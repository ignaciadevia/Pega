...
#DELITOS ASOCIACIÓN ILICITA
delitos_asociacion <- c("ASOCIACION ILICITA", "ASOCIACIONES ILICITAS LEY DE DROGAS (ART. 22)")
datos <- datos %>%
  mutate(delito_asociacion = if_else(rowSums(across(starts_with("DELITOS_RECEP"), ~ . %in% delitos_asociacion)) > 0, 1, 0))
...
datos <- datos %>%
  mutate(complejidad = if_else(delito_droga == 1 & delito_asociacion == 1, 1, 0))

datos <- datos %>%
  mutate(complejidad2 = if_else(delito_droga == 1 & delito_armas == 1, 1, 0))

datos <- datos %>%
  mutate(complejidad3 = if_else(delito_droga == 1 & delito_armas == 1 & delito_asociacion == 1, 1, 0))

table(datos$delito_droga, datos$Año)
table(datos$delito_asociacion)
table(datos$complejidad)
table(datos$complejidad2)
table(datos$complejidad3)

#PORCENTAJE POR AÑO
resultados <- datos %>%
  group_by(Año) %>%  # Agrupar por la columna de año
  summarise(
    total_ingresos = n(),
    total_droga = sum(delito_droga, na.rm = TRUE),
    total_armas = sum(delito_armas, na.rm = TRUE),
    total_asociacion = sum(delito_asociacion, na.rm = TRUE),
    total_complejidad = sum(complejidad, na.rm = TRUE),
    total_complejidad2 = sum(complejidad2, na.rm = TRUE),
    total_complejidad3 = sum(complejidad3, na.rm = TRUE)
    
  ) %>%
  mutate(
    porcentaje_droga = (total_droga / total_ingresos) * 100,
    porcentaje_armas = (total_armas / total_ingresos) * 100,
    porcentaje_asociacion = (total_asociacion / total_ingresos) * 100,
    porcentaje_complejidad = (total_complejidad / total_ingresos) * 100,
    porcentaje_complejidad2 = (total_complejidad2 / total_ingresos) * 100,
    porcentaje_complejidad3 = (total_complejidad3 / total_ingresos) * 100
  )

#PORCENTAJES POR REGION

resultados_region <- datos %>%
  group_by(REGION, Año) %>%  # Agrupar por región y año
  summarise(
    total_ingresos = n(),
    total_droga = sum(delito_droga, na.rm = TRUE),
    total_armas = sum(delito_armas, na.rm = TRUE),
  ...
  arrange(Año, REGION)  # Ordenar primero por Año y luego por Región

write_xlsx(
  list(
    "Porcentajes Nacionales" = resultados,
    "Porcentajes por Región y Año" = resultados_region
  ), 
  "porcentajes_delitos1.xlsx"
)
...
#DELITOS ASOCIACIÓN ILICITA
delitos_asociacion <- c("ASOCIACION ILICITA", "ASOCIACIONES ILICITAS LEY DE DROGAS (ART. 22)", "ASOCIACION ILICITA ART. 28  LEY 19.913")
datos <- datos %>%
  mutate(delito_asociacion = if_else(rowSums(across(starts_with("DELITOS_RECEP"), ~ . %in% delitos_asociacion)) > 0, 1, 0))
...
#DELITOS ASOCIACIÓN ILICITA
delitos_asociacion <- c("ASO", "ASOCIACION ILICITA", "ASOCIACIONES ILICITAS LEY DE DROGAS (ART. 22)", "ASOCIACION ILICITA ART. 28  LEY 19.913")
datos <- datos %>%
  mutate(delito_asociacion = if_else(rowSums(across(starts_with("DELITOS_RECEP"), ~ . %in% delitos_asociacion)) > 0, 1, 0))
